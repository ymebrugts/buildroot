...
#else  // UNICODE
typedef struct tagIBSU_SdkVersionW
{
	wchar_t Product[IBSU_MAX_STR_LEN];                    /* Product version string. */         
    wchar_t File[IBSU_MAX_STR_LEN];                       /* File version string. */
}
IBSU_SdkVersionW;

#define IBSU_SdkVersion IBSU_SdkVersionW
#endif
...
#else  // UNICODE
typedef struct tagIBSU_DeviceDescW
{
    wchar_t serialNumber[IBSU_MAX_STR_LEN];               /* Device serial number. */
    wchar_t productName[IBSU_MAX_STR_LEN];                /* Device product name. */
    wchar_t interfaceType[IBSU_MAX_STR_LEN];              /* Device interface type (USB, Firewire). */
    wchar_t fwVersion[IBSU_MAX_STR_LEN];                  /* Device firmware version. */
    wchar_t devRevision[IBSU_MAX_STR_LEN];                /* Device revision. */
    int   handle;                                       /* Return device handle. */
    BOOL  IsHandleOpened;                               /* Check if device handle is opened. */
#ifdef __android__
    int  devID;                                        /* Device ID. */
...
#define IBSU_DeviceDesc IBSU_DeviceDescW
#endif
